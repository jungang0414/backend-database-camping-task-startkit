...
FROM 
	"COURSE_BOOKING"
WHERE user_id = (SELECT id FROM "USER" WHERE "name" = '王小明')
AND status != '課程已取消' -- 除了課程已取消, 其他狀態會使用到堂數
GROUP BY user_id;
...
WHERE s."name" = '重訓'
ORDER BY c.experience_years ASC;
...
INNER JOIN "COACH" c ON cls.coach_id = c.id 
INNER JOIN "USER" u ON c.user_id = u.id
WHERE u."role" = 'COACH' -- 與COACH表關聯並確認是否為教練身分
GROUP BY s."name" -- 
ORDER BY coach_total DESC -- 遞減排序
LIMIT 1;
...
FROM
	"CREDIT_PURCHASE" cpc
INNER JOIN "CREDIT_PACKAGE" cp ON cpc.credit_package_id = cp.id
WHERE cpc.created_at BETWEEN '2024-12-01' AND '2024-12-31'
GROUP BY cp."name";
...
SELECT  
	COUNT(DISTINCT(user_id)) AS 預約會員人數 -- 排除重複項 並計算總數
FROM
	"COURSE_BOOKING"
WHERE status != '課程已取消'
AND
booking_at BETWEEN '2024-11-01' AND '2024-11-30';